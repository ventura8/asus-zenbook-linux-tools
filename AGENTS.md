...
- Tag push `v*` runs [`.github/workflows/ppa-release.yml`](.github/workflows/ppa-release.yml):
  ...
  not an ancestor of the repository default branch.
...
- Packaging lives under [`debian/`](debian/). Payload is installed to
  ...
  `python3-evdev (>= 1.6.0)` / `python3-dbus` / `python3-yaml` under
  `Build-Depends-Indep` (Architecture: all). Package `Depends` lists unversioned
  `python3` (no `>= 3.13` floor) and pins `python3-evdev (>= 1.6.0)` for
  `InputDevice.absinfo`.
...
- Every `--compat-only` matrix lane must run
  ...
  enabled).
  Dockerfile `rocky-9` uses `FROM rockylinux/rockylinux:9@sha256:…` (pinned digest)
  and must not hard-require `ydotool` or `alsa-tools`. Do **not** rely on platform
  `python3-gobject` (Python 3.9 site-packages) for CI `python3.13`: poetry installs
