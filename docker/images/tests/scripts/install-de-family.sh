...
_dnf_packages_for_family() {
    ...
    case "$FAMILY" in
        kde)
            case "$id" in
                rhel)
                    # Legacy EL9 rhel: kf6-kconfig absent; kf5 provides kwriteconfig5.
                    printf '%s' "kf5-kconfig"
                    ;;
                *)
                    # Rocky 10 / Alma / Fedora: kf6-kconfig provides kwriteconfig6.
...
_el_xfce_needs_source_build() {
    ...
    case "$id" in
        # EL10: EPEL has libxfce4util but not xfconf (Rocky/Alma/RHEL).
        rocky|almalinux|rhel) return 0 ;;
        *) return 1 ;;
    esac
}
