...
- Example single distro lane command:
  ...
  ```bash
  set -euo pipefail && sudo mkdir -p reports/distro-logs && \
  sudo timeout 3600 ./scripts/run_docker_matrix.sh --compat-only --distro rocky:9 \
  2>&1 | sudo tee reports/distro-logs/rocky-9.log
  ```
...
Rocky 9 Full-DE KDE uses `kf5-kconfig` / `kwriteconfig5` (no KF6). Alma/RHEL 10
